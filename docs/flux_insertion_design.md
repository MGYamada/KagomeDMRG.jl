...
2026-09-20の[研究方針の再検討](research/research_direction_20260920.md)では、
長い円筒の競合する中央構造と境界依存を主題にした。
2026-09-23にコードベース整備を先行する方針へ変更し、追加の研究計算は保留した。
現在の開発順序・研究再開の判断は[ロードマップ](../ROADMAP.md)を正本とする。
本書の物理・数値規約と精度条件は維持し、以下の研究計画を直ちに実行する指示とはしない。
...
有限円筒の磁場境界をこの値に合わせない。研究再開後はθ=0の静的NN研究と
既知CSLのポンプ校正を進める。文献の競合候補・予算・段階別の判定は
...
供給されたsector energy表からこの下側包絡を求める`magnetization_curve`を実装した。
全入力Qを比較し、磁化ジャンプ・一点のみの縮退・未計算Qを保存する。
[APIと小系検証の範囲](magnetization_curve.md)。この解析は追加DMRGやflux追跡を行わず、
有限系の磁化曲線からbulk plateau・Hall応答を認定するものではない。
...
この保存後診断を`static_diagnostics`と`diagnose_checkpoint`に共通化した。
実際の模型・Q・theta・gauge・hzを引き継ぎ、測定完了・整合性・精度を別々に記録する。
現行source・環境の小系で直接測定と別process測定を照合した。
[API契約と検証範囲](static_diagnostics.md)を参照する。旧checkpointの由来は変更しない。
...
研究再開後の静的なSz・bond pattern比較はポンプ校正の完了を待つ必要はない。固定Qでは横磁化の一体期待値がゼロなので、
...
続く[同一親・各1 sweepのχ256/512比較](research/p4_q5_matched_chi_comparison.md)では、
χ512で分散と切断が改善したが全5精度条件は未達だった。十分なsweep数で収束したχ依存と
区別し、Q1/Q3固定の有限trial区間への感度として記録する。
続く[χ512追加計算](research/p4_static512_sector_followup.md)では、Q1を累積9へ、
Q5を固定χで累積10へ進めた。solve・状態保存と保存状態診断を別process・別上限で完了し、
solveのみのrecordに未測定分散やintegrityを後付けせず、両段階をhashで結んだ。
全5精度条件は未達で、既存Q3 χ512累積9と合わせた区間は約0.258104–0.483571 J。
Q5の分散改善と切断誤差悪化を両方保持し、相同定やポンプ校正とは区別する。
...
その保存trialを追加DMRGなしで[別の分散後処理](research/p4_q5_matched_chi_comparison.md)にかけ、
123.524秒で0.24237056 J²を取得した。親から約0.681%増え、CSL開始基準は未達。
旧時間切れrecordと新診断を分けて保持し、非零ポンプ校正は未達のままとする。
NN静的研究では、N27を一時保留していた間にN18・Q6の追加で既存Q2境界が不変であることを確認した。
保存済みQ2の近接中性二重項も、縮退基底に不変な密度・bond遷移強度を測定した。
この小系の追加再加工を自動継続せず、NNの静的sector・秩序比較と
別模型のCSL零flux精度改善は研究再開時の候補として保持する。
再開時には各計算前に別の予算を固定し、全組合せ走査を条件にしない。
...
ユーザーが指定した候補の規約は以下のとおり。ロードマップから数値設計の本節へ集約する。
...
表は指定したK,tの予測で、微視的模型の実現を仮定しない。縮退数はトポロジカルな寄与のみで、
対称性の破れによる縮退を含めない。符号はfluxと移送方向の規約に依存する。
