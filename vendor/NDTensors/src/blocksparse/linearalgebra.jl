# Modified for KagomeDMRG on 2026-09-19: share global truncation selection
# between retained block ranks and Spectrum for SVD and Hermitian eigen.
# See truncate_spectrum.jl. General non-Hermitian eigen is unchanged.
using .Expose: expose
...
function svd(
    ...
    if any(!isnothing, (maxdim, cutoff))
        blockdims, d, truncerr = _truncate_block_spectrum(
            [abs2.(data(cpu(S))) for S in Ss], nzblocksT;
            min_blockdim, mindim, maxdim, cutoff, use_absolute_cutoff, use_relative_cutoff
        )
        for n in 1:nnzblocks(T)
            blockdim = blockdims[n]
            if blockdim == 0
                push!(dropblocks, n)
            else
...
function LinearAlgebra.eigen(
    ...
    if any(!isnothing, (maxdim, cutoff))
        if T isa Hermitian
            blockdims, d, truncerr = _truncate_block_spectrum(
                [abs.(data(cpu(D))) for D in Ds], nzblocks(T);
                min_blockdim, mindim, maxdim, cutoff, use_absolute_cutoff, use_relative_cutoff
            )
        else
            # The general dense eigen kernel does not sort each block's
            # eigenpairs. Preserve its upstream behavior; selecting and
            # permuting general eigenvectors is outside this density-matrix fix.
            d, truncerr, docut = truncate!!(
                d; mindim, maxdim, cutoff, use_absolute_cutoff, use_relative_cutoff
            )
            blockdims = [_truncated_blockdim(
                D, docut; min_blockdim, singular_values = false, truncate = true
            ) for D in Ds]
        end
        for n in 1:nnzblocks(T)
            blockdim = blockdims[n]
