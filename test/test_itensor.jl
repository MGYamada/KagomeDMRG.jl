@testset "Complex U(1) MPO versus independent ED" begin
    ...
    for theta in (0.0, 0.37, -0.71), gauge in (:seam, :uniform)
    ...
    seam = full_mpo_matrix(twisted_exchange_mpo(sites, lattice, 0.37), sites)
...
@testset "9-site DMRG against independent ground spaces" begin
    for (theta, seed) in ((0.0, 11), (0.37, 11), (0.37, 29))
        ...
        @info "Independent ED/DMRG validation" m
...
@testset "Measured truncation on an analytic field control" begin
    sites = siteinds("S=1/2", 2; conserve_qns=true)
    terms = OpSum()
    terms += 1.0, "Sz", 1, "Sz", 2
    terms += 0.5, "S+", 1, "S-", 2
    terms += 0.5, "S-", 1, "S+", 2
    # An explicitly altered two-spin Hamiltonian with unequal Schmidt weights
    # avoids the upstream degenerate-cutoff issue recorded separately below.
    terms += -3/8, "Sz", 1
    terms += 3/8, "Sz", 2
    H = MPO(ComplexF64, terms, sites)
    initial = MPS(ComplexF64, sites, ["Up", "Dn"])
    ...
    # Ground energy -7/8, Schmidt probabilities (4/5,1/5). Keeping one
    # produces the Up,Dn product state with energy -5/8.
    @test length(observer.max_truncation_errors) == 1
    @test only(observer.max_truncation_errors) ≈ 0.2 atol=1e-14
    @test local_energy ≈ -0.875 atol=1e-14
    @test real(inner(truncated', H, truncated)) ≈ -0.625 atol=1e-14
end

@testset "Degenerate QN truncation either retains a valid state or fails explicitly" begin
    sites = siteinds("S=1/2", 2; conserve_qns=true)
    terms = OpSum()
    terms += 1.0, "Sz", 1, "Sz", 2
    terms += 0.5, "S+", 1, "S-", 2
    ...
    observer = KagomeDMRG._SweepDiagnostics()
    outcome = try
        dmrg(H, initial; nsweeps=1, maxdim=1, cutoff=0.0, noise=0.0,
             observer, outputlevel=0)
    catch exception
        exception
    end
    if outcome isa Exception
        # On the tested backend the observer catches an empty link before
        # Krylov sees a zero initial vector. A future upstream fix may succeed.
        @test outcome isa ErrorException
        @test occursin("factorization produced a zero or nonfinite state", sprint(showerror, outcome))
    else
        local_energy, truncated = outcome
        @test norm(truncated) ≈ 1 atol=1e-14
        @test only(observer.max_truncation_errors) ≈ 0.5 atol=1e-14
        @test local_energy ≈ -0.75 atol=1e-14
        @test real(inner(truncated', H, truncated)) ≈ -0.25 atol=1e-14
    end
end
...
@testset "Flux-independent longitudinal control" begin
    ...
    sites = spin_sites(lattice)
    labels = [fill("Up", 10); fill("Dn", 8)]
    hz = [fill(1.0, 10); fill(-1.0, 8)]
    baseline = MPS(ComplexF64, sites, labels)
    before = deepcopy(baseline)
    result = run_dmrg(lattice, 0.73; sites, psi0=baseline, hz,
