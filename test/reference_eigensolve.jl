...
function _reference_eigensolver_settings(dimension::Integer;
        nev::Integer=3, tol::Real=1e-11, krylovdim::Integer=80,
        maxiter::Integer=200, seed::Integer=1234,
        blocksize::Integer=max(nev, 2))
    2 <= dimension <= 48620 || throw(ArgumentError("reference dimension must lie in 2:48620"))
    ...
    return (; nev=Int(nev), tol=Float64(tol),
...
function _reference_low_eigensystem(H::AbstractMatrix;
    ...
    settings = _reference_eigensolver_settings(dimension;
...
"""
    reference_eigensystem(Lx, Ly, theta=0.0;
        nup=nothing, Q=nothing, nev=3, tol=1e-11, krylovdim=80, maxiter=200, seed=1234,
        blocksize=max(nev, 2))

Find the lowest `nev` eigenpairs of the independent, sparse, nearest-neighbor
isotropic Heisenberg Hamiltonian in seam gauge. The default sector is
`M/Msat=1/9`; explicit `Q` or `nup` follows `reference_hamiltonian` and the
two cannot be combined. At most 18 sites and dimension 48620 (`N=18,Q=0`)
are supported, with bounds checked before constructing the Hamiltonian.
The low-eigensolver requires `1≤nev<dimension`, so saturated `Q=±N` sectors
are rejected even though their Hamiltonians can be constructed. Include
`reference_ed.jl` before this file; no production MPO or DMRG routine is used.
...
function reference_eigensystem(Lx::Integer, Ly::Integer, theta::Real=0.0;
        nup=nothing, Q=nothing, nev::Integer=3, tol::Real=1e-11, krylovdim::Integer=80,
        maxiter::Integer=200, seed::Integer=1234,
        blocksize::Integer=max(nev, 2))
    N = _reference_site_count(Lx, Ly)
    sector = _reference_sector(N; nup, Q)
    isfinite(theta) || throw(ArgumentError("theta must be finite"))
    _reference_eigensolver_settings(sector.dimension;
        nev, tol, krylovdim, maxiter, seed, blocksize)
    reference = reference_hamiltonian(Lx, Ly, theta; nup=sector.nup, sparse_matrix=true)
