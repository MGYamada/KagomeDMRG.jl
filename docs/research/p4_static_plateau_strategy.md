...
この全sector比較を行う解析APIとして[`magnetization_curve`](../magnetization_curve.md)を実装した。
入力energyの下側包絡、正の幅の区間・一点のみの縮退・飛び越しを返し、未計算Qを明示する。
sampleの近接判定許容差は区間の幅を変更せず、入力の数値誤差棒にも使わない。
bare energy表だけから模型・由来・収束を照合するものではなく、旧研究recordの由来も変更しない。
